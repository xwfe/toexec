...
*Small shared Rust crates for AI coding-agent execution layers: the pure mechanisms that both gld and ccnm need — bounded line reading and durable atomic file replacement. Zero dependencies (one crate uses serde_json), pinned by tag. Docs are in Chinese.*
...
- **要按单个工具、单个 skill 管给 AI 看什么。** 用户在 `~/.agents/mcp.json` 里写一次，gld 和 ccnm 照同一套规则收窄。
...
| `toexec-skill` | 读 `SKILL.md`：拆 frontmatter 并按 Claude Code 的读法读成键值、按它的规则替换 `$ARGUMENTS` / `$0` / `$name`、找出 `` !`命令` `` 注入（只找不跑） | `toexec-skill-v0.2.0` |
| `toexec-agents` | 读 `~/.agents/mcp.json`：某个产品的某个工具给不给、某个 skill 给到哪一档（[RFC-0001](docs/rfc/0001-agents-exposure-policy.md)） | `toexec-agents-v0.1.0` |

前三个 crate **没有任何依赖**，只用标准库；`toexec-agents` 只依赖 `serde_json`（两个产品本来就链接同一版）。
...
toexec-skill = { git = "https://github.com/xwfe/toexec.git", tag = "toexec-skill-v0.2.0" }
toexec-agents = { git = "https://github.com/xwfe/toexec.git", tag = "toexec-agents-v0.1.0" }
```
...
| 看 `~/.agents/mcp.json` 的格式、四档 skill 和为什么这样定 | [RFC-0001](docs/rfc/0001-agents-exposure-policy.md) |
