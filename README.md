...
*Small shared Rust crates for AI coding-agent execution layers: the pure mechanisms that both gld and ccnm need — bounded line reading, durable atomic file replacement, SKILL.md parsing, and relaying already-installed MCP servers. Pinned by tag. Docs are in Chinese.*
...
- **要把用户在 Claude Code / Codex 里装好的 MCP server 转给别的客户端。** 配置在两个文件里、写法不同；server 说的协议版本不一样（还有停在 2024-11-05 的）；一次结果能有 839 KB，其中一半是重复的 `structuredContent`。
...
| `toexec-mcp` | 转发装好的 MCP server：读 `.mcp.json` / `~/.claude.json` / `~/.codex/config.toml`、握手调用（接 2024-11-05 到 2025-11-25）、子进程通道（有界读行、能超时）、连接池（用到才开、闲了收）、结果整理（只去掉确定是副本的结构化结果、大文字整段交给产品分段）、长结果留在内存里按段读、拆 SSE 回复。进程怎么起怎么杀、HTTP 请求怎么发留在产品里 | `toexec-mcp-v0.2.1` |
...
前三个 crate **没有任何依赖**，只用标准库。`toexec-mcp` 例外：它要 serde_json 和 toml——JSON-RPC 和两种配置文件格式就是它处理的东西本身，理由见[开发规矩](docs/development.md#往这里抽东西的规矩)第 3 条。
...
toexec-mcp   = { git = "https://github.com/xwfe/toexec.git", tag = "toexec-mcp-v0.2.1" }
```
...
| 照着例子把这几个 crate 用起来，弄清边界和常见坑 | [docs/usage.md](docs/usage.md) |
...
| 看 gld / ccnm 怎么用上两台机器上装好的 skills 和 MCP server | [docs/plan/implementation-plan-v4-machine-skills-mcp.md](docs/plan/implementation-plan-v4-machine-skills-mcp.md) |
