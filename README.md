...
| `toexec-mcp` | 转发装好的 MCP server：读 `.mcp.json` / `~/.claude.json` / `~/.codex/config.toml`、握手调用（接 2024-11-05 到 2025-11-25）、子进程通道（有界读行、能超时）、连接池（用到才开、闲了收）、结果整理（去重复、大文字整段交给产品分段）。进程怎么起怎么杀、HTTP 通道留在产品里 | `toexec-mcp-v0.1.0` |
...
toexec-mcp   = { git = "https://github.com/xwfe/toexec.git", tag = "toexec-mcp-v0.1.0" }
```
