...
**当前状态：第一个共享 crate 已经在用。** `crates/wk-text` 提供有界行读取（`next_line`），gld 和 ccnm 都按 tag 链接它——两边的 `read_file` 契约不一样、**不会统一**，共用的只有「读一行但不把整行读进内存」这一个原语。哪些东西重复、哪些只是看起来重复，逐项对照在 [重复度盘点](evidence/v2-k/duplication-audit.md)。
...
许可证 MIT（见 [LICENSE](LICENSE)）。`wk-text` 没有任何依赖，所以暂时没有第三方来源要记。
