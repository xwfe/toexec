...
日期：2026-09-15。状态：**实施中**——V2-K、V2-H、V2-Q、V2-C 四条主线已完成（V2-C 于 2026-09-16 经 ccnm P24 真机验收），Claude 实验线 V2-P0、V2-P1 已完成，**V2-P1 结论是维持直接执行，实验线结束**（V2-P2–P5 不进入）；逐项进度见第 11 节的状态表，**计划原文与实际执行的偏移和处理见第 11 节"对齐检查"**。**2026-09-17 用户决定封存 V2-C 的产品实现**（ccnm P32）：三种客户端都走 MCP + 共享库一条路，最终目标和当前覆盖表见第 0 节。
...
本版是当前方案讨论入口。v1 原文（`docs/plan/implementation-plan.md`，状态一直是“未开始实施”）已于 2026-09-20 删除，要差异追溯去 git 历史里找；它的“已确认”“已获同意”和阶段编号从来不构成 v2 的实施或费用授权。两个产品的实际进度仍分别由各自仓库记录，特别是 ccnm 的 `docs/plan/status.json`。
...
### 0.1 最终目标：三种客户端 × 三种操作系统（用户 2026-09-17 明确）

Claude Code、Codex CLI、Web AI（经 gld hub）三种客户端，在 macOS、Linux、Windows 三种操作系统上都能用同一条执行路径。封存原生链就是为了只剩一条路径可做跨平台。**这是目标，不是计划**：Windows 那一列还没有任何设计，要另立 RFC。当前覆盖（汇总；事实以 ccnm `docs/support-matrix.md` 和 gld `docs/install.md` 为准，两边有出入以那两处为准）：

| 客户端 | macOS | Linux | Windows |
| --- | --- | --- | --- |
| Claude Code（ccnm 受管 / 外部 MCP） | Agent 与 Runtime 都真机验过 | 只验过当 Runtime（Debian 13 x86_64，外部 MCP 真项目 dogfood）；当 Agent 未验收 | 不支持 |
| Codex CLI（ccnm MCP 路径） | Agent 与 Runtime 都真机验过 | 当 Runtime 未在 MCP 路径上单独验过（P24 在 Debian 上验的是已封存的原生链）；当 Agent 未验收 | 不支持 |
| Web AI（gld hub → ccnm bridge） | hub 与 Runtime 跨两台真机验过（两台都是 macOS） | gld 本地二进制实机验过；hub → ccnm → Linux Runtime 没有用 gld 验过（ccnm 在 Debian 13 上验过的外部 MCP，Host 是 Claude Code） | gld 本地每次发版构建、CI 编译检查，未真机跑过；hub 连远端要本机有 ccnm，而 ccnm 不跑 Windows |

Windows 缺的不是一个平台分支：ccnm 的 Agent 半边靠 launchd 和 tmux，Runtime 半边靠 sshd、Unix 进程组和 `ps` / `/proc`；gld 有 Windows 的命名管道实现但没真机跑过。先定 Windows 当 Agent、当 Runtime、还是两者都要，再谈方案。
...
| V2-C Codex 原生路线 | 保留官方原生 exec-server 接入，独立锁定客户端/服务端组合和监督器（**2026-09-17 封存**：ccnm P32，见第 0 节） | 原生链的协议、权限、执行位置、guard和恢复门禁通过；不以 Claude 试验通过为前提 | 离线 0；真实回合需明确预算 |
...
**累计（2026-09-23）：25/145 次。**V2-Q2 用 20 次（Claude，$2.1454），ccnm P24 用 3 次（Codex/ChatGPT 订阅，Codex 不报费用；实验单上限 5），ccnm 2026-09-20 真机轮 1 次（Claude，$0.57，ccnm `docs/research/real-machine-p36-p44-2026-09-20.md`；当时漏记在这里），ccnm P50 补验 1 次（Claude，`claude-opus-5`，$0.23，ccnm P50 记录的补验一节）。以后记账只改这一行。
...
**执行到哪**：四条主线 V2-K、V2-H、V2-Q、V2-C 都已完成，第 13 节缺陷队列清空；Claude 实验线 V2-P0–P5 一项没开始。计划本身不要求 Claude 线先于主线，所以这不是滞后，而是**下一步要不要开 V2-P0 需要用户决定**。主线剩下的只有下表里标"未做"的几项。**2026-09-17 更新**：V2-P0/P1 做完并否决，V2-C 封存（第八批）；表里第 7–9 行已测（第七批）；主线还没做的只剩父目录 fsync。
...
2026-09-17 第七批：ccnm P29 补测上表第 7–9 行，零额度、本机 macOS，真实 `exec-serve` + 真实 Codex 0.154.0 exec-server + 不 import ccnm 的中立客户端（脚本在 `evidence/v2-c/p29-gates/`）。三项门禁都有了结果；查出 fs helper（exec-server 做带沙箱文件读写时起的子进程，环境被清空）在 exec-server 被强杀时活过放锁，ccnm P30 随即修掉：收尾扫描把 exec-server 进程组里剩下的进程也算进去（Linux 上 helper 不在这个组里，靠 bubblewrap 的 die-with-parent，未实测）。模型额度 0 次。
