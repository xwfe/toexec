...
| V2-C | **第一步 V2-G05 候选方案否决**，停在"只做合成数据实验"，等用户选方向 | `evidence/v2-c/g05/README.md`：Codex 0.154.0 真实 exec-server + 本机假模型接口 + 禁出站沙箱，零模型请求。一次性 URL 令牌能挡住错误令牌和重放；但断线后令牌会进入发给模型的工具输出、会话记录和 stderr。另确认 Codex 启动时就连 exec-server，断线后以 100 ms 间隔重连 25 秒 |
