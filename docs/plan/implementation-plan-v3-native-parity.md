...
3. **一行 shell**：`exec_command` 加一个和 `cmd` 二选一的参数，等价于 `sh -c`，省得模型自己拼 argv。不因此改任何权限判断——现在模型本来就能写 `["sh","-c",…]`。
...
1. **WebFetch 放不放开。** 它是从跑 CLI 的机器——也就是持有 AI 凭据的那台——直接出网抓网页，能碰到那台机器的内网。WebSearch 在厂商服务端执行，没有这个问题。建议：WebSearch 默认放开，WebFetch 做成 opt-in。
2. **Agent 面放开是默认行为还是按 workspace opt-in。** 它改变的是受管会话一直以来"只有七个工具"的承诺。建议：opt-in 一个版本，对照实验没问题后再改默认。
