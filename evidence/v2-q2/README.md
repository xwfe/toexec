...
2026-09-16 在 ccnm 立 P15 阶段落地（提交 `db53098`，纯文档）：外部入口的
`mcpServers` 示例加了 `"alwaysLoad": true`，协议文档里有一小节讲依据、不写会
怎样和代价（首轮请求前要等 bridge 连上 Runtime）。Managed 路径按上面的结论
不改。ccnm 那边另做了一次零额度复现（`coding` 21→14、`read` 18→14，后者是本
轮没测的模式），记录在 ccnm 仓库 `docs/research/p15-alwaysload-2026-09-16.md`。
